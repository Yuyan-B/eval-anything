...
class TV2ACTBenchmark(BaseBenchmark):
    ...
    def save_benchmark_details(self, save_path: str, benchmark_name: str,results_lst: List, details_lst :List):
        ...
        with open(os.path.join(video_dir, f"details_{self.task_type}.jsonl"), 'a', encoding='utf-8') as f:
        ...
        for detail in tqdm(details_lst):
            ...
            save_frames_to_mp4(
                frames=detail["navigation_video_frames"], file_path=navigation_video_path_to_send, fps=5
            )
            save_frames_to_mp4(
                frames=detail["manipulation_video_frames"], file_path=manipulation_video_path_to_send, fps=5
            )
            save_frames_to_mp4(
                frames=detail["third_person_video_frames"], file_path=third_person_video_path_to_send, fps=5
            )
    ...
    def batch_inference(self, agent, input_data: list[InferenceInput]) -> list[InferenceOutput]:
        ...
        for input_task in tqdm(input_data):
            ...
            input_task.embodied_task = self.task_sampler.next_task(force_advance_scene=True, house_index=int(input_task.metadata["house_id"]))
            input_task.max_steps = self.pre_defined_max_steps
            # 2. evaluate
            output = self.evaluate_on_task(input_task=input_task, agent=agent)
            # 3. get output
            task_info = {**input_task.embodied_task.task_info, **input_task.embodied_task.task_info["eval_info"]}
    ...
    def evaluate_on_task(self, input_task: InferenceInput, agent: AbstractAgent):
        ...
        with torch.no_grad():
            while len(all_actions) < input_task.max_steps:
                ...
                curr_frame = np.concatenate(
                    [input_task.embodied_task.controller.navigation_camera, input_task.embodied_task.controller.manipulation_camera], axis=1
                )
                ...
                top_down_curr_frame, third_person_curr_frame = input_task.embodied_task.controller.get_real_time_top_down_and_third_person_path(input_task.embodied_task.task_info["followed_path"])
                top_down_video_frames.append(top_down_curr_frame)
                navigation_video_frames.append(input_task.embodied_task.controller.navigation_camera)
                manipulation_video_frames.append(input_task.embodied_task.controller.manipulation_camera)
        ...
        if "synset_to_object_ids" in input_task.embodied_task.task_info:
            target_ids = list(
                chain.from_iterable(input_task.embodied_task.task_info.get("synset_to_object_ids", None).values())
            )
        top_down_frame, agent_path = get_top_down_frame(
            input_task.embodied_task.controller, input_task.embodied_task.task_info["followed_path"], target_ids
        )
