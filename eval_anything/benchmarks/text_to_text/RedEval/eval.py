...
from eval_anything.benchmarks.text_to_text.RedEval.utils import (
...
@BenchmarkRegistry.register('RedEval')
class RedEval(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        return {'RedEval': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['RedEval']
        llm_response = inference_outputs['RedEval']
        eval_response = gpt_evaluate(jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir)
        for output_item, eval_item in zip(inference_outputs['RedEval'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        jailbreak_count = 0
        all_valid_count = 0
        for output_item in inference_outputs['RedEval']:
        ...
        self.display_benchmark_results(self.benchmark_name, {"RedEval": {"Attack Success Rate": {"default": asr}}})

        return (
            inference_outputs,
            {"RedEval": {"Attack Success Rate": asr}},
            {},
        )
