...
from eval_anything.benchmarks.text_to_text.gptfuzzer.utils import EVALUATE_PROMPT, parse_json
...
class gptfuzzer(T2TBenchmark):
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset(
            'json',
            data_dir='benchmarks/text_to_text/gptfuzzer',
            data_files='prompts.json',
            split='train',
        )
        ...
        return {'gptfuzzer': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['gptfuzzer']
        llm_response = inference_outputs['gptfuzzer']
        eval_response = gpt_evaluate(
            jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir
        )

        for output_item, eval_item in zip(inference_outputs['gptfuzzer'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        for output_item in inference_outputs['gptfuzzer']:
            if not output_item:
                continue
