...
from eval_anything.benchmarks.text_to_text.HExPHI.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
class HExPHI(T2TBenchmark):
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        for item in all_prompts:
            inference_inputs.append(
                InferenceInput(
                    task="HExPHI",
                    conversation=[{"role": "system", "content": input_sys_prompt}, {"role": "user", "content": greetings % item}],
                    ref_answer=None,
                    metadata={
                        "prompt": item,
                    },
    ...
    def to_EvaluationResult(
        ...
        self.display_benchmark_results(self.benchmark_name, {"HExPHI": {"Harmfulness Rate": {"default": harmfulness_rate}}})
