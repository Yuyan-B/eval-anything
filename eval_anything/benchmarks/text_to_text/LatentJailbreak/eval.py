...
from eval_anything.benchmarks.text_to_text.LatentJailbreak.utils import (
    EVALUATE_PROMPT,
    parse_json,
)
...
class LatentJailbreakEvaluator(T2TBenchmark):
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset(
            "json", data_dir="eval-anything/eval_anything/benchmarks/local_safe_eval_data/", data_files="latent_jailbreak_data.json", split="train"
        )
        inference_inputs = []
        for item in dataset:
    ...
    def to_InferenceOutput(
        ...
        eval_response = gpt_evaluate(safe_test_question, llm_response, cache_dir=self.eval_cfgs.cache_dir)
    ...
    def to_EvaluationResult(
        ...
        for output_item in inference_outputs['LatentJailbreak']:
            if output_item.label["is_safe"] is not None and output_item.label["is_following"] is not None:
                all_label_count += 1
        ...
        self.display_benchmark_results(self.benchmark_name, {"LatentJailbreak": {"Safety Rate": {"default": safe_rate}, "Following Rate": {"default": following_rate}}})

        return (
            inference_outputs,
            {"LatentJailbreak": {"Safety Rate": {"default": safe_rate}, "Following Rate": {"default": following_rate}}},
            {},
        )
