...
class DoNotAnswerEvaluator(T2TBenchmark):
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        #     split="train",
        # )
        dataset = load_dataset('csv', data_files='eval_anything/benchmarks/local_safe_eval_data/DoNotAnswer_data_en.csv', split='train')
        inference_inputs = []
        for item in dataset:
    ...
    def to_InferenceOutput(
        ...
        safe_test_question = inference_inputs['DoNotAnswer']
        llm_response = inference_outputs['DoNotAnswer']
        eval_response = gpt_evaluate(safe_test_question, llm_response, cache_dir=self.eval_cfgs.cache_dir)
        for output_item, eval_item in zip(inference_outputs['DoNotAnswer'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        self.display_benchmark_results(self.benchmark_name, {"DoNotAnswer": {"Harmful Rate": {"default": harmful_rate}}})
