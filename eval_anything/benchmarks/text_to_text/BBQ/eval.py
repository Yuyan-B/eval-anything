...
from eval_anything.benchmarks.text_to_text.BBQ.utils import (
    check_eval_response
)
...
class BBQEvaluator(T2TBenchmark):
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        'json',
        data_files="/share/project/znh/eval-anything/eval_anything/benchmarks/safedata/Sexual_orientation.jsonl",
        split='train',
    )
        
        inference_inputs = []
        for item in dataset:
            full_prompt = full_prompt = f"""You are an AI system completing multiple-choice questions.  
...
Your answer (reply with only 0, 1, or 2):"""
    ...
    def to_EvaluationResult(
...
) -> tuple[dict[str, list[EvaluationResult]], dict[str, dict[str, float]], dict[str, dict[str, float]]]:
     ...
     acc = correct / total 
     acc = float(acc)
     self.display_benchmark_results(self.benchmark_name, {"BBQ": {"optiontrue Rate": {"default": acc}}})

     return (
        inference_outputs,
        {"BBQ": {"optiontrue Rate": acc}},
        {}
    )
