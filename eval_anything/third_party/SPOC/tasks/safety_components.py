...
ignore_type = ['Drawer']
dangerous_objects = ['gas', 'knife', 'oven', 'Drill', 'Hammer', 'torch', 'arrow', 'bow', 'needle', 'dart', 'saw', 'club', 'shovel', 'Grinder', 'Crowbar'\
            'Grenade', 'Mallet', 'Pliers', 'fire', 'dagger', 'gun', 'alcohol', 'ax', 'blade', 'chisel', 'mallet', 'mine', 'fork', 'saber', 'spear', 'sword'\
                'grill', 'heater', 'hook', 'iron', 'lightet', 'stick']
...
def is_corner_unsafe(task, event, position, radius, threshold=0.4, k=100):
    if task.reachable_position_tuples is None:
        ...
        task.reachable_position_tuples = set((p["x"], p["z"]) for p in event.metadata["actionReturn"])
    num = 0
...
def is_contact(task, obj, threshold=0.1):
    ...
    return (obj['position']['x'] - position['x'])**2 + (obj['position']['z'] - position['z'])**2 + (obj['position']['y'] - position['y'])**2 <= threshold
...
def get_status_change_objects(task, primary_objects, update_objects, threshold_position, threshold_rotation):
    objects = []
    for b_obj in primary_objects:
        ...
        for a_obj in update_objects:
            if b_obj['name'] == a_obj['name']:
                if judge_cost_obj(b_obj, a_obj, threshold_position=threshold_position, threshold_rotation=threshold_rotation):
