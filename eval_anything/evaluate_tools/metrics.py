...
class FalsePositiveRate(BaseMetric):
    def calculate(self, evaluation_results: List[EvaluationResult], judge_method: str, **kwargs: None) -> dict[str, float]:
        ...
        for evaluation_result in evaluation_results:
            for extractor, extracted_result in evaluation_result.extracted_result.items():
                if judge(extracted_result, positive_label) and not judge(evaluation_result.ground_truth, positive_label):
...
class FalseNegativeRate(BaseMetric):
    def calculate(self, evaluation_results: List[EvaluationResult], judge_method: str, **kwargs: None) -> dict[str, float]:
        ...
        for evaluation_result in evaluation_results:
            for extractor, extracted_result in evaluation_result.extracted_result.items():
                if not judge(extracted_result, positive_label) and judge(evaluation_result.ground_truth, positive_label):
...
class TruePositiveRate(BaseMetric):
    def calculate(self, evaluation_results: List[EvaluationResult], judge_method: str, **kwargs: None) -> dict[str, float]:
        ...
        for evaluation_result in evaluation_results:
            for extractor, extracted_result in evaluation_result.extracted_result.items():
                if judge(extracted_result, positive_label) and judge(evaluation_result.ground_truth, positive_label):
...
class TrueNegativeRate(BaseMetric):
    def calculate(self, evaluation_results: List[EvaluationResult], judge_method: str, **kwargs: None) -> dict[str, float]:
        ...
        for evaluation_result in evaluation_results:
            for extractor, extracted_result in evaluation_result.extracted_result.items():
                if not judge(extracted_result, positive_label) and not judge(evaluation_result.ground_truth, positive_label):
